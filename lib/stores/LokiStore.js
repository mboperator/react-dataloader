  ...
  return {
    ...
    add(object, trigger=true) {
      ...
      if (stale) {
        ...
        // if already exists. dont add it, just sync it.
        this.trigger("add", object, trigger);
      } else {
        // ===========================
        // TESTING TEMP IDS
        // ===========================
        // TODO we need a way to provide tmp_IDs to objects if we
        // don't persist them on the DB (!trigger)
     ...
     * @return {event} Triggers event
     */
    destroy(object, trigger=true) {
      var toDestroy;
      if (typeof object === "object") { toDestroy = object[idAttr] }
      else { toDestroy = object }
      this.collection.remove(this.get(toDestroy));
      this.trigger("destroy", object, trigger);
    },
