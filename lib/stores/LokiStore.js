 ...
 */
    filter(tag, filterQuery) {
      ...
      var view = this.dynamicViews[tag];
      view.filter = filterQuery;
      this._generateDynamicView(tag, view);
    },
/**
 ...
 */
    sort(tag, sortBy) {
      ...
      viewParams.sortedBy = sortBy;
      this._generateDynamicView(tag, viewParams);
    },
/**
 * Adds a sort parameter
 * @param {string} tag Tag of Collection to be filtered
 ...
 * @return {event} Triggers event
 */
    _generateDynamicView(tag, viewParams) {
      var view = this.collection.addDynamicView(tag);
      var { filter, sortedBy } = viewParams;
      view.applySimpleSort(sortedBy);
      view.applyFind(filter);

      this.trigger("dynamicViewUpdated", tag);
    }
  };
}
