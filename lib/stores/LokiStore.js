 ...
 */
    filter(tag, filterQuery) {
      if (!this.dynamicViews[tag]) { 
        this.dynamicViews[tag] = { filters: {}, sortBy: {} }; 
      }
      this.collection.removeDynamicView(tag);
      var viewParams = this.dynamicViews[tag];

      extend(viewParams.filters, filterQuery);
 ...
 */
    registerView(tag, viewParams) {
      ...
      if (!viewParams) { this.trigger("viewRegistered", tag); }

      var { filters, sortBy } = viewParams;
      sortBy && view.applySimpleSort(sortBy.attribute, sortBy.isDescending);

      var _applyFilter = function(key) {
      ...
      // Turn into function
      if (filters) {
        Object.keys(filters).forEach(_applyFilter);
      }
