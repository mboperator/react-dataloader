...
function generateStore(options) {
  var { name, idAttr, fk, columnDefinitions } = options;
  ...
  return {
    ...
    update(updates, trigger=true) {
      ...
      if (!stale) {
        ...
        this.trigger("update", updates, trigger, stale);
      }
    },
    ...
    filter(tag, filterQuery) {
      if (!this.dynamicViews[tag]) {
        this.dynamicViews[tag] = {
          filters: {}, sortBy: {}, search: "", columnDefinitions: this.columnDefinitions
        };
      }
...
function getPropAccessor(columnDefinitions, key) {
  debugger;
  let def = _.filter(columnDefinitions, (def) => def.key === key);
  return def.length ? def[0].textField : null;
}

function _buildFilter(key, value, columnDefinitions) {
  let formattedValue = String(value).toLowerCase();
  let propAccessor = getPropAccessor(columnDefinitions, key);
  return function(object) {
    var prop = propAccessor ? propAccessor(object) : object[key];
    let formattedProp = String(prop).toLowerCase();
    if (value === undefined) { return !object[key]; }
    return String(object[key]).toLowerCase() === formattedValue;
  };
}

function _applyViewFilters(view, filters, columnDefinitions) {
  Object.keys(filters).forEach((key) => {
    ...
    switch(filters[key]) {
      case null:
        break;
      default:
        query = _buildFilter(key, filters[key], columnDefinitions);
        break;
    }
