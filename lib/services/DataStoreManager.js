 ...
 */
  shared(options) {
    ...
    var client = this.client.bind(null, this.urlBuilder);
 ...
 */
  unique(options) {
    ...
    var chosenStore = new storeTemplates.unique(options);
    chosenStore.listenables = DataActionGenerator();
    var store = Reflux.createStore(chosenStore);
    var synchronizer = this.synchronizer(options, store, this.client);
    synchronizer.populate();
