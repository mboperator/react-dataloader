...
var Nsynchronizer = {
  ...
  setDeserializer: function(deserializer=defaultDeserializer) {
  ...
  _logError: function(method, err) {
    this.onError && this.onError(method, err);
  },
    ...
    this.client.getAll(object)
      .then((objects) => {
        ...
        if (objects.constructor === Array) {
          ...
          this.store.listenables.setObject(formatted);
        }
  ...
  * @public Push store additions to remote
  ...
  add: function(object) {
    ...
    .fail(err => {
      ...
      this.store.destroy(object[this.idAttribute], false);
      console.error(err);
    });
  },
};
