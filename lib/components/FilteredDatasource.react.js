...
var FilteredDatasource = React.createClass({
  ...
  _reloadCollection(props, state) {
    ...
    var nextData = _.clone(dataSource.getCollection(props.tag));
    var stateData = _.clone(this.state.data);
    var data = this._transform(nextData, stateData);
  ...
  _transform(nextData, stateData) {
    // // Guards for name
    if(!stateData) return nextData;
    
    var stateNames = {};
    
    stateData.forEach(function(obj){
      stateNames[obj.name] = obj;
    });

    var data = nextData.filter(function(obj){
        return !(obj.name in stateNames);
    });
